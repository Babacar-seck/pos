  ...
  template: `
    <app-sidebar>
      <div class="page-header">
        ...
        @if (
          activeTab() === 'products' && !showAddForm() && !editingProduct() && canEditProducts()
        ) {
          <div class="page-header-actions">
            ...
            @if (products().length > 0) {
              <button
                type="button"
                class="btn btn-secondary btn-delete-all"
          ...
          @if (showAddForm() || editingProduct()) {
            <div class="form-card">
              <div class="form-header">
                <h3>
                  {{
                    editingProduct()
                      ? ('PRODUCTS.EDIT_PRODUCT' | translate)
              ...
              <form (submit)="saveProduct($event)">
                <div class="form-row">
                  <div class="form-group">
                    ...
                    <input
                      ...
                      [class.input-error]="productFormErrors()?.name"
                      (ngModelChange)="clearProductFormError('name')"
                    />
                    @if (productFormErrors()?.name) {
                      <span class="field-error" role="alert">{{
                        'PRODUCTS.NAME_REQUIRED' | translate
                      }}</span>
                    }
                  </div>
                  <div class="form-group form-group-sm">
                    <label for="price"
                      >{{ 'PRODUCTS.PRODUCT_PRICE' | translate }}
                      <span class="required" aria-hidden="true">*</span></label
                    >
                    <div class="price-row-with-helper">
                      <div class="price-input">
                        ...
                        <input
                          ...
                          name="price"
                          required
                          [placeholder]="'PRODUCTS.PRICE_PLACEHOLDER' | translate"
                          [readonly]="!canEditProducts()"
                          [class.input-error]="productFormErrors()?.price"
                          (ngModelChange)="clearProductFormError('price')"
                        />
                      </div>
                      @if (canEditProducts()) {
                        <button
                          type="button"
                          class="btn btn-secondary btn-sm pricing-ideal-btn"
                          (click)="openPricingHelper()"
                        >
                    ...
                    @if (productFormErrors()?.price) {
                      <span class="field-error" role="alert">{{
                        'PRODUCTS.PRICE_REQUIRED' | translate
                      }}</span>
                    }
                  </div>
                  <div class="form-group form-group-sm">
                    ...
                    <div class="price-input">
                      ...
                      <input
                        ...
                        name="cost"
                        [placeholder]="'PRODUCTS.COST_PLACEHOLDER' | translate"
                        [readonly]="!canEditProducts()"
                      />
                    </div>
                ...
                <div class="form-group">
                  ...
                  <input
                    ...
                    name="ingredients"
                    [placeholder]="'PRODUCTS.INGREDIENTS_PLACEHOLDER' | translate"
                    [readonly]="!canEditProducts()"
                  />
                </div>
                <div class="form-group">
                  ...
                  <textarea
                    ...
                    name="description"
                    [placeholder]="'PRODUCTS.DESCRIPTION_PLACEHOLDER' | translate"
                    rows="3"
                    [readonly]="!canEditProducts()"
                  ></textarea>
                </div>
                ...
                <div class="form-group">
                  <label for="kitchen_station_id">{{
                    'PRODUCTS.KITCHEN_STATION_LABEL' | translate
                  }}</label>
                  <select
                  ...
                  >
                    <option [ngValue]="null">
                      {{ 'PRODUCTS.KITCHEN_STATION_NONE' | translate }}
                    </option>
                  ...
                  <small class="field-hint">{{
                    'PRODUCTS.KITCHEN_STATION_HINT' | translate
                  }}</small>
                </div>
                <div class="form-group">
                ...
                <div class="form-row">
                  <div class="form-group">
                    ...
                    <input
                      id="available_from"
                      type="date"
                      [(ngModel)]="formData.available_from"
                      name="available_from"
                      [readonly]="!canEditProducts()"
                    />
                    <small class="field-hint">{{
                      'PRODUCTS.AVAILABLE_FROM_HINT' | translate
                    }}</small>
                  </div>
                  <div class="form-group">
                    <label for="available_until">{{
                      'PRODUCTS.AVAILABLE_UNTIL' | translate
                    }}</label>
                    <input
                      id="available_until"
                      type="date"
                      [(ngModel)]="formData.available_until"
                      name="available_until"
                      [readonly]="!canEditProducts()"
                    />
                    <small class="field-hint">{{
                      'PRODUCTS.AVAILABLE_UNTIL_HINT' | translate
                    }}</small>
                  </div>
                </div>
                <div class="form-group">
                  ...
                  <small class="field-hint">{{
                    'PRODUCTS.IMAGE_UPLOAD_HINT' | translate: { maxMb: maxImageUploadMb }
                  }}</small>
                </div>
                ...
                @if (canEditProducts() && editingProduct()?.id) {
                  <div class="form-group product-questions-section">
                    ...
                    @if (questionsLoading()) {
                      <p class="questions-loading">
                        {{ 'PRODUCTS.QUESTIONS_LOADING' | translate }}
                      </p>
                            ...
                            <li class="question-row">
                              <div class="question-row-main">
                                <span class="question-type-badge">{{
                                  'PRODUCTS.QUESTION_BADGE_' + q.type.toUpperCase() | translate
                                }}</span>
                                @if (q.multi && q.type === 'choice') {
                                  <span class="question-type-badge question-multi-badge">{{
                                    'PRODUCTS.QUESTION_BADGE_MULTI' | translate
                                  }}</span>
                                }
                                <span class="question-label-text">{{ q.label }}</span>
                                @if (q.required) {
                                  <span class="question-required-badge">{{
                                    'PRODUCTS.QUESTION_REQUIRED_BADGE' | translate
                                  }}</span>
                                }
                              </div>
                              <div class="question-row-actions">
                                <button
                                  ...
                                  [disabled]="questionSaving() || idx === 0"
                                  [title]="'PRODUCTS.QUESTION_MOVE_UP' | translate"
                                >
                                  ↑
                                </button>
                                <button
                                  ...
                                  (click)="moveQuestion(idx, 1)"
                                  [disabled]="
                                    questionSaving() || idx === sortedProductQuestions().length - 1
                                  "
                                  [title]="'PRODUCTS.QUESTION_MOVE_DOWN' | translate"
                                >
                                  ↓
                                </button>
                                <button
                                  type="button"
                                  class="btn btn-ghost btn-sm"
                                  (click)="openQuestionEditor(q.id)"
                                  [disabled]="questionSaving()"
                                >
                                  {{ 'PRODUCTS.QUESTION_EDIT' | translate }}
                                </button>
                                <button
                                  type="button"
                                  ...
                                  [disabled]="questionSaving()"
                                >
                                  {{ 'PRODUCTS.QUESTION_DELETE' | translate }}
                                </button>
                              </div>
                            </li>
                          }
                      ...
                      <button
                        ...
                        (click)="openQuestionEditor('new')"
                        [disabled]="questionSaving()"
                      >
                        {{ 'PRODUCTS.QUESTION_ADD' | translate }}
                      </button>
                    } @else {
                      <div class="question-editor-card">
                        <h4 class="question-editor-title">
                          {{
                            questionEditorMode() === 'new'
                              ? ('PRODUCTS.QUESTION_NEW' | translate)
                              : ('PRODUCTS.QUESTION_EDIT_TITLE' | translate)
                          }}
                        </h4>
                        <div class="form-row">
                          <div class="form-group">
                            <label for="q-type">{{
                              'PRODUCTS.QUESTION_FIELD_TYPE' | translate
                            }}</label>
                            <select
                              id="q-type"
                              name="q-type"
                              [(ngModel)]="questionDraft.type"
                              [disabled]="questionSaving()"
                            >
                              <option value="choice">
                                {{ 'PRODUCTS.QUESTION_TYPE_CHOICE' | translate }}
                              </option>
                              <option value="scale">
                                {{ 'PRODUCTS.QUESTION_TYPE_SCALE' | translate }}
                              </option>
                              <option value="text">
                                {{ 'PRODUCTS.QUESTION_TYPE_TEXT' | translate }}
                              </option>
                            </select>
                          </div>
                          <div class="form-group flex-grow">
                            <label for="q-label">{{
                              'PRODUCTS.QUESTION_FIELD_LABEL' | translate
                            }}</label>
                            <input
                              id="q-label"
                              type="text"
                              name="q-label"
                              [(ngModel)]="questionDraft.label"
                              [disabled]="questionSaving()"
                              [placeholder]="'PRODUCTS.QUESTION_LABEL_PLACEHOLDER' | translate"
                            />
                          </div>
                        </div>
                        @if (questionDraft.type === 'choice') {
                          <div class="form-group">
                            <label for="q-options">{{
                              'PRODUCTS.QUESTION_FIELD_OPTIONS' | translate
                            }}</label>
                            <textarea
                              id="q-options"
                              name="q-options"
                              rows="4"
                              [(ngModel)]="questionDraft.choiceLines"
                              [disabled]="questionSaving()"
                              [placeholder]="'PRODUCTS.QUESTION_OPTIONS_PLACEHOLDER' | translate"
                            ></textarea>
                          </div>
                          <div class="form-group checkbox-row">
                            <label>
                              <input
                                type="checkbox"
                                name="q-multi"
                                [(ngModel)]="questionDraft.choiceMulti"
                                [disabled]="questionSaving()"
                              />
                        ...
                        @if (questionDraft.type === 'scale') {
                          <div class="form-row">
                            <div class="form-group form-group-sm">
                              <label for="q-min">{{
                                'PRODUCTS.QUESTION_SCALE_MIN' | translate
                              }}</label>
                              <input
                                id="q-min"
                                type="number"
                                name="q-min"
                                [(ngModel)]="questionDraft.scaleMin"
                                [disabled]="questionSaving()"
                              />
                            </div>
                            <div class="form-group form-group-sm">
                              <label for="q-max">{{
                                'PRODUCTS.QUESTION_SCALE_MAX' | translate
                              }}</label>
                              <input
                                id="q-max"
                                type="number"
                                name="q-max"
                                [(ngModel)]="questionDraft.scaleMax"
                                [disabled]="questionSaving()"
                              />
                            </div>
                          </div>
                        ...
                        <div class="form-group checkbox-row">
                          <label>
                            <input
                              type="checkbox"
                              name="q-req"
                              [(ngModel)]="questionDraft.required"
                              [disabled]="questionSaving()"
                            />
                        ...
                        <div class="question-editor-actions">
                          ...
                          <button
                            ...
                            [disabled]="questionSaving()"
                          >
                            {{ 'PRODUCTS.QUESTION_SAVE' | translate }}
                          </button>
                        </div>
                      </div>
                    }
                ...
                @if (canEditProducts() && showAddForm() && !editingProduct()?.id) {
                  <p class="field-hint product-questions-hint-new">
                    {{ 'PRODUCTS.CUSTOMIZATIONS_AFTER_SAVE' | translate }}
                  </p>
                }
                    ...
                    {{
                      saving()
                        ...
                        : editingProduct()
                          ...
                          : ('PRODUCTS.ADD_PRODUCT_BUTTON' | translate)
                    }}
                  </button>
                </div>
              </form>
            ...
            @if (loading()) {
              ...
              <div class="search-row">
                <label for="products-search" class="visually-hidden">{{
                  'PRODUCTS.SEARCH_PLACEHOLDER' | translate
                }}</label>
                <input
                  id="products-search"
                  type="search"
                ...
                @if (selectedCategory() && availableSubcategoriesForFilter().length > 0) {
                  <div class="ribbon-container subribbon">
                    <div class="ribbon">
                      ...
                      >
                        {{
                          'PRODUCTS.ALL_ITEMS_IN'
                            | translate: { category: getCategoryLabel(selectedCategory()!) }
                        }}
                      </button>
                        ...
                        <tr>
                          ...
                          <td>
                            @if (editingCategoryProductId() === product.id && canEditProducts()) {
                              ...
                              <span
                                class="category-cell"
                                [class.clickable]="canEditProducts()"
                                (click)="canEditProducts() && startCategoryEdit(product, $event)"
                              >
                          ...
                          <td>
                            @if (editingCategoryProductId() === product.id && canEditProducts()) {
                              <select
                                ...
                                [disabled]="
                                  !editingCategory ||
                                  getSubcategoriesForCategory(editingCategory || '').length === 0
                                "
                                (blur)="saveCategoryInline(product)"
                                (keydown.escape)="cancelCategoryEdit()"
                              >
                                <option value="">None</option>
                                @for (
                                  subcat of getSubcategoriesForCategory(editingCategory);
                                  track subcat
                                ) {
                                  <option [value]="subcat">
                          ...
                          <td class="actions">
                            <div class="actions-inner">
                              <button
                                ...
                                [attr.title]="'PRODUCTS.EDIT_TOOLTIP' | translate"
                              >
                                <svg
                                  width="16"
                                  height="16"
                              ...
                              @if (canEditProducts()) {
                                <button
                                  ...
                                  [attr.title]="'PRODUCTS.DELETE_TOOLTIP' | translate"
        ...
        @if (productToDelete()) {
          <div class="modal-overlay" (click)="productToDelete.set(null)">
            <div class="modal" (click)="$event.stopPropagation()">
              ...
              <p>
                {{
                  'PRODUCTS.DELETE_PRODUCT_CONFIRM' | translate: { name: productToDelete()?.name }
                }}
              </p>
              <div class="modal-actions">
                <button class="btn btn-secondary" (click)="productToDelete.set(null)">
                  {{ 'PRODUCTS.CANCEL' | translate }}
                </button>
                <button class="btn btn-danger" (click)="deleteProduct()">
                  {{ 'PRODUCTS.DELETE_PRODUCT' | translate }}
                </button>
              </div>
            </div>
          </div>
          ...
          <div class="modal-overlay" (click)="showDeleteAllModal.set(false)">
            <div class="modal" (click)="$event.stopPropagation()">
              ...
              <div class="modal-actions">
                <button
                  class="btn btn-secondary"
                  (click)="showDeleteAllModal.set(false)"
                  [disabled]="deletingAll()"
                >
                  {{ 'PRODUCTS.CANCEL' | translate }}
                </button>
                <button
                  ...
                  [disabled]="deletingAll()"
                >
                  {{ 'PRODUCTS.DELETE_ALL_BUTTON' | translate }}
                </button>
              </div>
            </div>
          </div>
...
export class ProductsComponent implements OnInit {
  ...
  canEditProducts = computed(() =>
    this.permissions.hasPermission(this.permissions.getCurrentUser(), 'product:write'),
  );
  ...
  ngOnInit() {
    ...
    this.api
      .getTaxes(false)
      .subscribe({
        next: (list) => this.productTaxes.set(list),
        error: () => this.productTaxes.set([]),
      });
  ...
  startEdit(product: Product) {
    ...
    this.formData = {
      name: product.name,
      price: toDisplayAmount(product.price_cents, this.decimalPlaces()),
      cost:
        product.cost_cents != null
          ? toDisplayAmount(product.cost_cents, this.decimalPlaces())
          : null,
  ...
  saveQuestionEditor() {
    ...
    if (mode === 'new') {
      ...
      const nextOrder = list.length === 0 ? 0 : Math.max(...list.map((q) => q.sort_order), 0) + 1;
      this.api
        ...
        .subscribe({
          ...
          error: (err) => {
            this.error.set(
              err.error?.detail || this.translate.instant('PRODUCTS.QUESTION_SAVE_FAILED'),
            );
            this.questionSaving.set(false);
          },
        });
  ...
  moveQuestion(index: number, dir: -1 | 1) {
    ...
    this.api.reorderProductQuestions(pid, ids).subscribe({
      ...
      error: (err) => {
        this.error.set(
          err.error?.detail || this.translate.instant('PRODUCTS.QUESTION_REORDER_FAILED'),
        );
        this.questionSaving.set(false);
      },
    });
  ...
  confirmDeleteQuestion(q: ProductQuestionStaff) {
    ...
    this.api.deleteProductQuestion(pid, q.id).subscribe({
      ...
      error: (err) => {
        this.error.set(
          err.error?.detail || this.translate.instant('PRODUCTS.QUESTION_DELETE_FAILED'),
        );
        this.questionSaving.set(false);
      },
    });
  ...
  saveProduct(event: Event) {
    ...
    const productData: Record<string, unknown> = {
      ...
      price_cents: toMinorUnits(this.formData.price, this.decimalPlaces()),
      cost_cents:
        this.formData.cost != null && this.formData.cost >= 0
          ? toMinorUnits(this.formData.cost, this.decimalPlaces())
          : undefined,
    ...
    if (editing?.id) {
      this.api.updateProduct(editing.id, productData as Partial<Product>).subscribe({
        ...
        error: (err) => {
          this.error.set(err.error?.detail || 'Failed to update');
          this.saving.set(false);
        },
      });
    } else {
      this.api.createProduct(productData as unknown as Product).subscribe({
          ...
          if (pendingFile && product.id) {
            ...
            this.api.uploadProductImage(product.id, pendingFile).subscribe({
              ...
              error: (err) => {
                this.error.set(
                  err.error?.detail ||
                    this.translate.instant('PRODUCTS.PRODUCT_CREATED_BUT_IMAGE_FAILED'),
                );
                this.clearPendingImage();
                this.uploading.set(false);
              },
        ...
        error: (err) => {
          this.error.set(err.error?.detail || 'Failed to create');
          this.saving.set(false);
        },
      });
    }
  ...
  deleteAllProducts() {
    ...
    this.api.deleteAllProducts().subscribe({
      ...
      error: (err) => {
        this.error.set(
          err.error?.detail || this.translate.instant('PRODUCTS.FAILED_TO_DELETE_ALL'),
        );
        this.deletingAll.set(false);
      },
    });
  ...
  deleteProduct() {
    ...
    this.api.deleteProduct(product.id).subscribe({
      ...
      error: (err) => {
        this.error.set(err.error?.detail || 'Failed to delete');
        this.deleting.set(null);
      },
    });
  }
